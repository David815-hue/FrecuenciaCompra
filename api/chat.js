...
- dateFilter: object or null. Extract date ranges when mentioned.
  * startDate: "YYYY-MM-DD" or null. IMPORTANT: Today is ${todayReadable} (${todayISO}).
    - For relative filters like "últimos 2 meses" (last 2 months), calculate the start date relative to Today (e.g. today minus 60 days -> ${new Date(now.getTime() - 60 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]}).
    - If the user says "últimos meses" (last few months) without specifying a number, assume a default of the last 3 months (90 days) relative to Today.
